...
def build_layout():
    ...
    layout = [[sg.Text('Choose files to build from')],
              ...
              [sg.Text('Merging style:', size=(TEXT_OUTLINE_SIZE)), sg.Combo(MERGING_STYLES_LIST\
                                                                             ...
                                                                             readonly=True), \
               sg.Text('Start Page:', key='txt_start_page', visible=False),
               sg.InputText(key='input_start_page', size=(5), visible=False), sg.Text('Stop Page:', key='txt_stop_page',visible=False),
               sg.InputText(key='input_stop_page', size=(5), visible=False)],
...
def main():
    ...
    while True:
        ...
        if event == sg.WIN_CLOSED or event == 'Exit':  # close the program
            ...
            if values['options_list'] == 'Middle Merger' or values['options_list'] == 'Cut':  # display the additional options
                window['txt_start_page'].update(visible=True)
                window['input_start_page'].update(visible=True)
                ...
                window['second_input_file'].update(visible=True)
                window['second_browse'].update(visible=True)
                window['input_start_page'].update('')  # reset the chosen page number
                window['input_stop_page'].update('')  # reset the chosen page number
                start_page = None
            ...
            merging_style = {'Sequence': Sequence, 'Middle Merger': Middle, 'Odd - Even': odd_even, 'Cut': cut}
            new_file_name = merge_functions.merge(first_path, second_path, saving_path=saving_path,
                                                  first_pdf_leads=True, User_named=user_named_file, merging_style=merging_style, first_middle_pause_page=start_page, stop_page=stop_page)
